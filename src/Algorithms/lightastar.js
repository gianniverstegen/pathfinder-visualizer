function lightastar(noNeighbourGrid, startNode, endNode, noAnimation) {
  if (noAnimation) {
  ...
  while (openList.length > 0) {
    ...
    if (current === endNode) {
      ...
      let astarPath = constructPath(current);
      return [visitedNodes, grid, astarPath];
    }
...
function getManhattenDistance(rowx, rowy, colx, coly) {
  return 0.75 * (Math.abs(rowx - rowy) + Math.abs(colx - coly));
}
...
function constructPath(current) {
  ...
  return astarPath.reverse(); // reversed leads away from the startnode instead of to.
}
